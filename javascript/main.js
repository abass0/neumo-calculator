...
document.querySelectorAll(".num__key").forEach(
    el => {
        el.onclick = () => input.value = input.value    //concatenate the text
        !== "0" ? input.value + el.innerText :
        el.innerText;
    }
)

const buffer = []; //"visor" of the calculator
...
for (const opName of ["add", "subtract", "multiply", "divide", "percent"]){
    document.querySelector(`.op__key[op=${opName}]`)                    //loop the operations + onclick event listener
